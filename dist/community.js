...
(function () {
  ...
  function buildAccount() {
    ...
    acctBtn.addEventListener('click', function (e) { e.stopPropagation(); acctOpen = !acctOpen; renderAccount(); });
  ...
  function renderAccount() {
    ...
    if (u && !u.needsProfile) {
      ...
      acctBtn.innerHTML = '<span style="width:28px;height:28px;border-radius:999px;border:1.5px solid ' + INK + ';background:' + LIME + ';display:flex;align-items:center;justify-content:center;font-weight:800;font-family:\'Bricolage Grotesque\',sans-serif;font-size:14px;">' + esc(initial) + '</span><span>' + esc((u.name || '').split(' ')[0]) + '</span>';
    } else {
    ...
    if (!acctOpen) return;
    acctMenu.appendChild(menuItem('💬  Community forum', function () { location.href = 'Forum.dc.html'; }));
    if (u && !u.needsProfile) {
      ...
      acctMenu.appendChild(menuItem('🔑  Sign in / Sign up', function () { openModal(); }));
    }
  }
  ...
  function boot() { buildAccount(); emit(); }
