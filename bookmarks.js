...
(function () {
  ...
  function build() {
    ...
    var reveal = function () {
      ...
      // MOBILE: dock as an icon-only ★ beside the "Join the Community" button, fixed to the
      // bottom bar (no scroll-follow), fading in/out on the same 3s timer as that button.
      if (window.matchMedia('(max-width: 640px)').matches) {
        launcher.innerHTML = '★';
        launcher.style.top = 'auto';
        launcher.style.right = 'auto';
        launcher.style.bottom = '20px';
        launcher.style.width = '46px';
        launcher.style.height = '46px';
        launcher.style.padding = '0';
        launcher.style.justifyContent = 'center';
        launcher.style.fontSize = '20px';
        var comm = document.querySelector('button[aria-label="Join the community"]');
        var starSize = 46, gap = 8, margin = 8;
        var leftPx = comm
          ? Math.min(comm.getBoundingClientRect().right + gap, window.innerWidth - starSize - margin)
          : (window.innerWidth - starSize - margin);
        launcher.style.left = leftPx + 'px';
        launcher.style.opacity = '1';
        launcher.style.pointerEvents = 'auto';
        clearTimeout(stubT); clearTimeout(hideT);
        hideT = setTimeout(function () { launcher.style.opacity = '0'; launcher.style.pointerEvents = 'none'; }, 3000);
        return;
      }

      // DESKTOP: reset any mobile overrides, then scroll-anchor along the right edge.
      ...
      var h = launcher.offsetHeight || 44;
      var nav = document.querySelector('nav');
      var navBottom = nav ? nav.getBoundingClientRect().bottom : 0;
      var minTop = Math.max(14, navBottom + 12);
      var top = minTop + frac * ((vh - 18 - h) - minTop);
      var footer = document.querySelector('footer');
      if (footer) { var ft = footer.getBoundingClientRect().top; if (ft < vh) top = Math.min(top, ft - 14 - h); }
      ...
      launcher.style.pointerEvents = 'auto';
      clearTimeout(stubT); clearTimeout(hideT);
      stubT = setTimeout(function () { launcher.innerHTML = '★'; launcher.style.padding = '12px 15px'; }, 500);
      hideT = setTimeout(function () { launcher.style.opacity = '0'; launcher.style.pointerEvents = 'none'; }, 3000);
    };
